 ...
 *
 *****************************************************************************/
#define _RTL8851BU_HALINIT_C_
#include "../rtl8851b_hal.h"

static void _hal_pre_init_8851bu(struct rtw_phl_com_t *phl_com,
				 struct hal_info_t *hal_info,
				 struct hal_init_info_t *init_51bu)
{
	struct mac_ax_trx_info *trx_info = &init_51bu->trx_info;
	...
	init_51bu->ic_name = "rtl8851bu";
}

void init_hal_spec_8851bu(struct rtw_phl_com_t *phl_com,
...
enum rtw_hal_status hal_get_efuse_8851bu(struct rtw_phl_com_t *phl_com,
					 struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_51bu;

	_os_mem_set(hal_to_drvpriv(hal_info), &init_51bu, 0, sizeof(init_51bu));
	_hal_pre_init_8851bu(phl_com, hal_info, &init_51bu);

	return hal_get_efuse_8851b(phl_com, hal_info, &init_51bu);
}

enum rtw_hal_status hal_init_8851bu(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
...
void hal_deinit_8851bu(struct rtw_phl_com_t *phl_com,
		       struct hal_info_t *hal_info)
{
	/* free memory for hal */
...
enum rtw_hal_status hal_start_8851bu(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_51bu;
	...
	return hal_start_8851b(phl_com, hal_info, &init_51bu);
}

static void hal_deinit_misc_8851bu(struct hal_info_t *hal)
{

}
#ifdef CONFIG_WOWLAN
enum rtw_hal_status
hal_wow_init_8851bu(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
						struct rtw_phl_stainfo_t *sta)
{
	...
	return hal_wow_init_8851b(phl_com, hal_info, sta, &init_51bu);
}

enum rtw_hal_status
hal_wow_deinit_8851bu(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
							struct rtw_phl_stainfo_t *sta)
{
	struct hal_init_info_t init_51bu;
	struct mac_ax_trx_info *trx_info = &init_51bu.trx_info;
	...
	return hal_wow_deinit_8851b(phl_com, hal_info, sta, &init_51bu);
}
#endif /* CONFIG_WOWLAN */


enum rtw_hal_status hal_stop_8851bu(struct rtw_phl_com_t *phl_com,
				      struct hal_info_t *hal)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
...
u32 hal_hci_cfg_8851bu(struct rtw_phl_com_t *phl_com,
...
bool hal_recognize_int_8851bu(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_halt_c2h_handler_8851bu(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_watchdog_timer_handler_8851bu(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
u32 hal_int_hdler_8851bu(struct hal_info_t *hal)
{
	...
	/* <4> Halt C2H related */
	int_hdler_msk |= (hal_halt_c2h_handler_8851bu(hal, generalhandled) << 4);

	/* <4>watchdog timer related */
	int_hdler_msk |= (hal_watchdog_timer_handler_8851bu(hal, generalhandled) << 5);
...
enum rtw_hal_status
hal_mp_init_8851bu(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_51bu;
	...
	hal_status = hal_mp_init_8851b(phl_com, hal_info, &init_51bu);
...
enum rtw_hal_status
hal_mp_deinit_8851bu(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_51bu;
	...
	hal_status = hal_mp_deinit_8851b(phl_com, hal_info, &init_51bu);

	if (RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_mp_deinit_8851bu: status = %u\n", hal_status);
		return hal_status;
	}
...
bool
hal_mp_path_chk_8851bu(struct rtw_phl_com_t *phl_com, u8 ant_tx, u8 cur_phy)
{
